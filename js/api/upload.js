import { setDirty } from "@core/store.js";
import { bytesToBase64, isProbablyText } from "@core/encoding.js";
...
export async function saveUploadedEntry({ mode, cloned, path, data }) {
  ...
  if (!data.isBinary) {
    // dirty всегда хранит LF-версию. Иначе при fromLf получится \r\r\n.
    const lf = (data.content || "").replace(/\r\n/g, "\n").replace(/\r/g, "\n");
    setDirty(path, lf);
  }
